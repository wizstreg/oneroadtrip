...
class BookingManager {
  constructor(lang = 'fr', tripId = null, userId = null) {
    this.lang = lang;
    this.bookings = {}; // stepIdx -> array of bookings
    
    // Charger depuis rt-user si disponible
    if (tripId && userId) {
      const lsKey = `ort_user_content_${userId}_${tripId}`;
      const saved = localStorage.getItem(lsKey);
      if (saved) {
        try {
          const userData = JSON.parse(saved);
          Object.entries(userData).forEach(([stepKey, stepData]) => {
            const match = stepKey.match(/^step_(\d+)$/);
            if (match) {
              const stepIdx = parseInt(match[1]);
              if (stepData.bookings && Array.isArray(stepData.bookings)) {
                this.bookings[stepIdx] = stepData.bookings;
              }
            }
          });
          if (Object.keys(this.bookings).length > 0) {
            console.log('[BOOKINGS] ✅ Chargé depuis rt-user:', Object.keys(this.bookings).length, 'étapes');
          }
        } catch(e) {
          console.error('[BOOKINGS] ⚠️ Erreur chargement rt-user:', e);
        }
      }
    }
  }
...
function initBookingManager(lang = 'fr', tripId = null, userId = null) {
  // Récupérer tripId depuis params URL ou state si absent
  if (!tripId) {
    ...
    tripId = tripId || window.state?.tripId;
  }
  
  // Récupérer userId depuis Firebase si absent
  if (!userId && typeof firebase !== 'undefined' && firebase.auth) {
    const user = firebase.auth().currentUser;
    userId = user?.uid;
  }
  
  globalBookingManager = new BookingManager(lang, tripId, userId);
  console.log('[BOOKINGS] Init avec tripId:', tripId, 'userId:', userId ? '✓' : '✗');
}
